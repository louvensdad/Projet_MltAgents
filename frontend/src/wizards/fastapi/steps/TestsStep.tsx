...
import { TEST_OPTIONS } from "../fastApiConfig";
...
export default function TestsStep({ data, onChange }: Props) {
  const { t } = usePreferences();

  const toggle = (opt: string) => {
    const next = data.test_options.includes(opt)
      ? data.test_options.filter((o) => o !== opt)
      : [...data.test_options, opt];
    onChange({ test_options: next });
  };

  return (
    <div className="space-y-6 animate-in slide-in-from-right-4 duration-500">
      <h2 className="text-2xl font-bold text-foreground">{t("wizard.fastapi.step8")}</h2>
        ...
        {TEST_OPTIONS.map((opt) => (
          <button
            ...
            className={`px-5 py-3 rounded-xl border-2 text-sm font-semibold transition-all ${
              data.test_options.includes(opt)
