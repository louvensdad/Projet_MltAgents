...
export default function UxStep({ uxOptions, onToggle, t, options }: Props) {
  return (
    <div className="space-y-3">
      <p className="text-sm font-semibold text-gray-200">{t("wizard.static.ux_title")}</p>
      <div className="grid gap-3 md:grid-cols-2 lg:grid-cols-3">
        {options.map((key) => {
          const selected = uxOptions.includes(key);
          return (
            <button
              key={key}
