...
export function useLdcnAvatarMotion(
  ...
  return useMemo(() => {
    ...
    const bob = isActive
      ? {
          y: mood === "walking" ? [0, -2, 0] : mood === "celebrating" ? [0, -4, 0] : mood === "thinking" ? [0, -1, 0] : [0, -3, 0],
          scale: mood === "celebrating" ? [1, 1.04, 1] : mood === "warning" || mood === "error" ? [1, 0.99, 1] : [1, 1.01, 1],
          rotate: mood === "walking" ? [-1.2, 1.2, -1.2] : mood === "listening" ? [-0.5, 0.5, -0.5] : [0, 0.5, 0],
        }
      : { y: 0, scale: 1, rotate: 0 };

    const transition: Transition = isActive
      ? {
          duration: mood === "celebrating" ? 1.6 : mood === "walking" ? 2.2 : 3.2,
          repeat: Infinity,
          ease: "easeInOut",
        }
      : { duration: 0 };
    ...
    const pathMotion = isActive
      ? {
          strokeDashoffset: mood === "walking" ? [0, 18, 0] : [0, 8, 0],
