...
class DocsRegistry:
    def __init__(self):
        ...
        self.registry: Dict[str, List[Dict[str, Any]]] = {
            "spring_boot": spring_sources,
            "springboot": spring_sources,
            "fastapi": fastapi_sources,
            "static_site": static_sources,
            "angular": [
    ...
    def get_sources(self, stack: str) -> List[Dict[str, Any]]:
        normalized = (stack or "").strip().lower().replace("-", "_").replace(" ", "_")
        return self.registry.get(normalized, self.registry["static_site"])
