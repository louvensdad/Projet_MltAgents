...
def _build_prompt_answers(payload: dict[str, Any], stack_id: str) -> dict[str, Any]:
    ...
    base: dict[str, Any] = {
        ...
        "business_goal": payload.get("business_goal") or payload.get("project_description") or f"Deliver {project_name} with production-ready architecture.",
        "business_rules": _to_list(payload.get("business_rules")) or _to_list(payload.get("confirmed_business_rules")) or ["Validate input", "Protect access", "Audit critical operations"],
        "entities": _to_list(payload.get("entities")) or _to_list(payload.get("confirmed_entities")) or ["User", "AuditLog"],
        "features": _to_list(payload.get("features")) or _to_list(payload.get("confirmed_features")) or ["crud_api", "validation", "documentation"],
        "language": payload.get("language") or payload.get("project_language") or locale,
        "docker": payload.get("docker", True),
    }
    ...
    if stack_id == "spring_boot":
        ...
        base.update({
            ...
            "cache": payload.get("cache", "redis"),
            "monitoring": "actuator_prometheus" if observability else "actuator_only",
            "observability": "actuator_prometheus" if observability else "actuator",
            ...
            "tests": "junit" if tests else "junit",
    ...
    if stack_id == "fastapi":
        ...
        base.update({
            ...
            "workers": "celery" if any("celery" in item.lower() for item in workers) else "background_tasks",
            "cache": payload.get("cache", "redis"),
            "openapi": any("swagger" in item.lower() or "openapi" in item.lower() for item in docs_options) or True,
            "testing_strategy": "unit_integration" if tests else "unit_only",
            "tests": "pytest" if tests else "pytest",
